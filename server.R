...
#=================================Serendip Frequency plot=======================
  ...
  Master_data=reactive({
    ...
    transactions = head(transactions,300)
    ...
    ## til date present after this date transformed
    ...
    patterns2 = function(Seq){
      ...
      sup_2=sup_2%>%mutate(Patterns_of=2)
      sup_2
    }
    
    # =================================Patterns3====================================
    # Find ordered match
    patterns3 = function(Seq){
      Seq=Seq%>% group_by(SID)%>% mutate(two=string_fun_2(ITEMS))
      ...
      X3=as.data.frame(table(Seq$two)) # require two column
      ...
      sup_3=sup_3%>%mutate(Patterns_of=3)
      sup_3
    }
    
    #===============================Patterns 4 =====================================
    patterns4 = function(Seq){
      Seq=Seq%>% group_by(SID)%>% mutate(three=string_fun_3(ITEMS))
      ...
      sup_4=sup_4%>%mutate(Patterns_of=4)
      sup_4
    }
    
    #=========================Patterns 5============================================
    
    patterns5 = function(Seq){
      Seq=Seq%>% group_by(SID)%>% mutate(four=string_fun_4(ITEMS))
      # Find ordered match
      ...
      sup_5=sup_5%>%mutate(Patterns_of=5)
      sup_5
    }
    
    # Load data and process then run below codes
    ...
    #Two item set
    k2=2
    K2=OMdataFrame%>% filter(EID==k2, Freq>=sigma)
    I2_selected=as.vector(K2$ITEMS)
    
    # Three item set
    k3=3
    K3=OMdataFrame%>% filter(EID==k3, Freq>=sigma)
    
    # Four item set
    k4=4
    K4=OMdataFrame%>% filter(EID==k4, Freq>=sigma)
    
    # Five item set
    k5=5
    K5=OMdataFrame%>% filter(EID==k5, Freq>=sigma)
    #=========================================Create IF function====================
    #maxevent = 4
    if(maxevent >= 5) {
      R2=patterns2(Seq)
      ...
      R4=patterns4(Seq)
      R5=patterns5(Seq)
      results = rbind(R2,R3,R4,R5)
      results
    } else if(maxevent==4){
      R2=patterns2(Seq)
      R3=patterns3(Seq)
      R4=patterns4(Seq)
      results = rbind(R2,R3,R4)
      results
